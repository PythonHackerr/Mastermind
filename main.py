...
pygame.display.set_caption("Mastermind game")  # Set screen title
...
GAME_AI = AI(list(PLAYER_COLORS.keys()))
GAME = GameManager(list(PLAYER_COLORS.keys()))
...
def check_answer():
    ...
    if (SET_CODE == True):
        ...
        update_screen = True
        return
    print(PLAYER_GRID[SELECTED_ROW])
    print(GAME.get_code())
    score = GameManager.check_answer(
        PLAYER_GRID[SELECTED_ROW], GAME.get_code())
    GAME_AI.store_score(score)
...
def set_code():
    ...
    global SET_CODE
    easygui.msgbox("Now set your own code and press Enter!")
    restart_game()
    SET_CODE = True
...
def save_game():
    ...
    dictionary = {
        ...
        "possibilities": GAME_AI.possibilities
    }
...
def load_game():
    ...
    with open("data.json", "r") as file:
        ...
        GAME_AI.clear_history()
        GAME.set_code(data['code'])
        GAME_AI.possibilities = data['possibilities']
        GAME_AI.scores_history = data['score_grid']
        GAME_AI.answers_history = data['player_grid']
...
def restart_game():
    ...
    global update_screen, GAME_AI, PLAYER_GRID, SCORE_GRID, GAME, SELECTED_COLUMN, SELECTED_ROW
    PLAYER_GRID = [["blank"] * 4 for rows in range(8)]
    SCORE_GRID = [["blank"] * 4 for rows in range(8)]
    GAME = GameManager(list(PLAYER_COLORS.keys()))
    GAME_AI = AI(list(PLAYER_COLORS.keys()))
